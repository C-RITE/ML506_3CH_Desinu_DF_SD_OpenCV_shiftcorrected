// VideoChannelG.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CVideoChannelG, CDialog)

CVideoChannelG::CVideoChannelG(CWnd* pParent /*=NULL*/)
	: CDialog(CVideoChannelG::IDD, pParent)
{
...
void CVideoChannelG::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_DESINUSOID2, m_btnDesinusoid);
	DDX_Control(pDX, IDC_IMGSIZE, m_lblImageSize);
}


BEGIN_MESSAGE_MAP(CVideoChannelG, CDialog)
	ON_WM_PAINT()
	ON_BN_CLICKED(IDC_SHOW_HIST, &CVideoChannelG::OnBnClickedShowHist)
	ON_BN_CLICKED(IDC_DESINUSOID2, &CVideoChannelG::OnBnClickedDesinusoid2)
	ON_WM_LBUTTONDBLCLK()
	ON_WM_RBUTTONDBLCLK()
	ON_WM_LBUTTONDOWN()
	ON_WM_KEYDOWN()
...
// CVideoChannelG message handlers

BOOL CVideoChannelG::OnInitDialog()
{
...
LPBITMAPINFO CVideoChannelG::CreateDIB(int cx, int cy)
{
	LPBITMAPINFO lpBmi;
	int iBmiSize;
...
void CVideoChannelG::UpdateImageInfo(int width, int height, int offsetX, int offsetY, BOOL bBSflag) {
	int sizeX, sizeY;
...
/*		if (m_bDesinusoid) {
		...
		if (g_bDesinusoid) {
			...
			::StretchDIBits(m_hdc, m_iOffsetX, m_iOffsetY, m_imgWidth, m_imgHeight,
								0, 0, m_imgWidth, m_imgHeight, g_bSampleSW?g_imgBuffDF:m_desBuffer,
								m_BitmapInfoWarp, NULL, SRCCOPY);
		}
...
void CVideoChannelG::RefreshVideo() {
	OnPaint();
	Invalidate(FALSE);
}

void CVideoChannelG::OnCancel()
{
...
void CVideoChannelG::OnBnClickedShowHist()
{
...
void CVideoChannelG::MapHistCurves()
{
	int i, offset = 260;
...
void CVideoChannelG::OnBnClickedDesinusoid2()
{
	m_bDesinusoid = !m_bDesinusoid;
}

void CVideoChannelG::RunDesinusoid() {
	...
	if (g_bDesinusoid) {
		...
		for (j = 0; j < m_imgHeight; j ++) {
			...
			//	*(m_desBuffer+p+m_imgWidth-1-i) = (BYTE)val;
				g_bSampleSW?*(m_disBuffer+p+m_imgWidth-1-i) = ((*(g_imgBuffDF+p+idx1))*val1 + (*(g_imgBuffDF+p+idx2))*val2):0;
...
void CVideoChannelG::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	m_ptLoc.x = point.x;
	m_ptLoc.y = point.y;
...
void CVideoChannelG::OnRButtonDblClk(UINT nFlags, CPoint point)
{
	m_ptLoc.x = -1;
	m_ptLoc.y = -1;
...
void CVideoChannelG::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	switch(nChar){
