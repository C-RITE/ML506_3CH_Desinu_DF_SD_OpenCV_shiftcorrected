...
void ReadWritePipesMenu(HANDLE hDevice)
{
    ...
    do {
        ...
        printf("1.  Switch to %s mode\n", !fStreamMode ? 
            "Streaming Data Transfers" : "Single Blocking Transfers");
        printf("2.  Read from pipe\n");
        printf("3.  Write to pipe\n");
        printf("4.  Listen to pipe (continuous read)\n");
        if (fStreamMode)
        {
            printf("5.  Check streaming READ speed\n");
            printf("6.  Check streaming WRITE speed\n");
        }
        printf("99. Main menu\n");
        printf("Enter option: ");
        cmd = 0;
        ...
        if (cmd==99)
            break;
        if (cmd<1 || (!fStreamMode && cmd>4) || cmd>6)
            continue;
        ...
        case 3:
            ...
            else
            {
                dwError = WDU_Transfer(hDevice,
                    ...
                    cmd==2, 0, pBuffer, dwSize, &dwBytesTransferred,
                    SetupPacket, TRANSFER_TIMEOUT);
            }
