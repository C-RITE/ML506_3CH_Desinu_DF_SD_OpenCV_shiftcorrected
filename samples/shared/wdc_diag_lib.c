    ...
    FILE *fp)
{
    ...
    for (i = 0, resources = 0; i < pCard->dwItems; i++)
    {
        ...
        case ITEM_INTERRUPT:
            ...
            /* According to MSI specification, it's recommended that device will
             * support both MSI and Level Sensitive interrupts, and allow OS to
             * choose the interrupt type to work with. */
            ...
            {
                /* MSI/MSI-X interrupts are always edge-triggered, so there is
                 * no need to print that possibility */
                fprintf(fp, "        %s\n",
...
char *WDC_DIAG_IntTypeDescriptionGet(DWORD dwIntType)
{
    if (dwIntType & INTERRUPT_MESSAGE_X)
        return "Extended Message Signalled Interrupt (MSI-X)";
    else if (dwIntType & INTERRUPT_MESSAGE)
        return "Message Signalled Interrupt (MSI)";
