    ...
    BOOL fHex, DWORD dwSize, UINT64 min, UINT64 max)
{
    ...
    if (dwSize != sizeof(BYTE) && dwSize != sizeof(WORD) &&
        dwSize != sizeof(UINT32) && dwSize != sizeof(UINT64))
    {
        DIAG_ERR("DIAG_InputNum: Error - Invalid size (%ld)\n", dwSize);
        return DIAG_INPUT_FAIL;
    }

    iRet = sscanf(sInput, fHex ? "%"PRI64"x" : "%"PRI64"u", &tmp);
    ...
    if (fCheckRange && (tmp < min || tmp > max))
    {
