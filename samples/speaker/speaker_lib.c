...
BOOL SPEAKER_Open (SPEAKER_HANDLE *phSPEAKER)
{
    ...
    *phSPEAKER = NULL;
    ...
    dwStatus = WD_CardRegister(hSPEAKER->hWD, &hSPEAKER->cardReg);
    if (hSPEAKER->cardReg.hCard==0)
    {
        sprintf(SPEAKER_ErrorString, "Failed locking device. Status 0x%lx - %s\n",
...
void SPEAKER_Tone (SPEAKER_HANDLE hSPEAKER, DWORD dwHz, DWORD dwMilli)
{
    ...
    BZERO(sleep);
    sleep.dwMicroSeconds = dwMilli * 1000 ; // 2 seconds
