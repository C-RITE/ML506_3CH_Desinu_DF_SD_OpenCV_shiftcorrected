// VideoChannelR.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CVideoChannelR, CDialog)

CVideoChannelR::CVideoChannelR(CWnd* pParent /*=NULL*/)
	: CDialog(CVideoChannelR::IDD, pParent)
{
...
void CVideoChannelR::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_FPS_LABLE, m_lblFPS1);
	DDX_Control(pDX, IDC_SLIDER1, m_lblFPS2);
	...
	DDX_Control(pDX, IDC_DESINUSOID, m_btnDesinusoid);
	DDX_Control(pDX, IDC_IMGSIZE, m_lblImageSize);
}


BEGIN_MESSAGE_MAP(CVideoChannelR, CDialog)
	ON_WM_PAINT()
	ON_BN_CLICKED(IDC_SHOW_HIST, &CVideoChannelR::OnBnClickedShowHist)
	ON_BN_CLICKED(IDC_DESINUSOID, &CVideoChannelR::OnBnClickedDesinusoid)
	ON_WM_LBUTTONDBLCLK()
	ON_WM_RBUTTONDBLCLK()
	ON_WM_KEYDOWN()
	ON_WM_CTLCOLOR()
...
// CVideoChannelR message handlers

BOOL CVideoChannelR::OnInitDialog()
{
...
//	m_PenBack.CreatePen(PS_SOLID, 2, crBkgnd);
...
LPBITMAPINFO CVideoChannelR::CreateDIB(int cx, int cy)
{
	LPBITMAPINFO lpBmi;
	int iBmiSize;
...
void CVideoChannelR::UpdateImageInfo(int width, int height, int offsetX, int offsetY, BOOL bBSflag) {
	int sizeX, sizeY;
...
void CVideoChannelR::RefreshVideo() {
	OnPaint();
	Invalidate(FALSE);
}

void CVideoChannelR::OnCancel()
{
...
void CVideoChannelR::OnBnClickedShowHist()
{
...
void CVideoChannelR::MapHistCurves()
{
	int i, offset = 260;
...
void CVideoChannelR::OnBnClickedDesinusoid()
{
	m_bDesinusoid = !m_bDesinusoid;
	g_VideoInfo.bDesinusoid = m_bDesinusoid;
}


void CVideoChannelR::RunDesinusoid() {
	...
	if (g_bDesinusoid) {
		...
		for (j = 0; j < m_imgHeight; j ++) {
			...
			for (i = 0; i < m_imgWidth; i ++) {
				...
				val = (*(m_imgBuffer+p+idx1))*val1 + (*(m_imgBuffer+p+idx2))*val2;
...
void CVideoChannelR::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	m_ptLoc.x = point.x;
	m_ptLoc.y = point.y;
...
//		g_stimulus.y = point.y;
		...
		stripH     = -1;
		nChannelID = STIM_CHALL;
		g_objVirtex5BMD.AppWriteStimScaleY(g_hDevVirtex5, 0, g_VideoInfo.img_height, 0, 0, stripH, nChannelID);
		g_objVirtex5BMD.AppWriteStimLocX(g_hDevVirtex5, 0, 0, 0, 0, g_VideoInfo.img_width, g_VideoInfo.img_height, 0, 0, nChannelID);
		g_objVirtex5BMD.AppWriteStimScaleY(g_hDevVirtex5, point.y-g_VideoInfo.stim_ny/2, point.y+g_VideoInfo.stim_ny/2, 0, g_VideoInfo.stim_nx, stripH, nChannelID);
		g_objVirtex5BMD.AppWriteStimLocX(g_hDevVirtex5, 0, point.x, point.x, point.y-g_VideoInfo.stim_ny/2, point.y+g_VideoInfo.stim_ny/2, 0, x1, x2, nChannelID);
...
void CVideoChannelR::OnRButtonDblClk(UINT nFlags, CPoint point)
{
	m_ptLoc.x = -1;
	m_ptLoc.y = -1;

	int stripH, nChannelID;

	stripH     = -1;
	nChannelID = STIM_CH796;
	g_objVirtex5BMD.AppWriteStimScaleY(g_hDevVirtex5, 0, g_VideoInfo.img_height, 0, 0, stripH, nChannelID);
	g_objVirtex5BMD.AppWriteStimLocX(g_hDevVirtex5, 0, 0, 0, 0, g_VideoInfo.img_width, g_VideoInfo.img_height, 0, 0, nChannelID);
...
void CVideoChannelR::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	switch(nChar){
...
HBRUSH CVideoChannelR::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
